...
def random_point_dist(index: int, grid_settings: GridMapDefinition, expected_rate: float, max_population: int) -> tuple:
    ...
    return (
        ...
        gauss(mu=expected_rate, sigma=expected_rate * 0.5), # gaussian distribution, expected values follow the poisson (numpy) distribution
    )

def random_point_dist_pop(index: int, grid_settings: GridMapDefinition, expected_rate: float, max_population: int) -> tuple:
    """Generate a random data point with population."""
    return (
        index + 1, # location index
        random() * grid_settings.cols * grid_settings.grid_size, # X coordinate
...
def create_random_location_data(
    ...
    data_generator = data_generators['with_population'] if with_population else data_generators['without_population']

    rate_data = []
    for loc_id in range(number_of_locations):
        
        loc_data = data_generator(
            loc_id, 
            grid_settings, 
            expected_rate, 
            max_population
